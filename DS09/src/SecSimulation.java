...
public class SecSimulation {
	...
	private static void serve() {
		...
		for (int t = 0;; t++) {
			...
			for (int j = 0; j < numServers; j++) {
				...
				if (server.isIdle() && !queue[j].isEmpty()) {
	...
	private static int compareQ(){
		int q;
		if(queue[0].size()<=queue[1].size() && queue[0].size()<=queue[2].size()) return q=0;
		else if(queue[1].size()<queue[0].size() && queue[1].size()<=queue[2].size()) return q=1;
		else return q=2;
	}
}
